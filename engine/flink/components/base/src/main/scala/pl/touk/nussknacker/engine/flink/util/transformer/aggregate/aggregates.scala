...
object aggregates {
  ...
  /*
    Aggregator that holds two smaller aggregators and can add elements
    to one of them. Adding Left(v) causes v to be added to first aggregator,
    adding Right(v) causes v to be added to second aggregator. Result outputs
    pair (result of first aggregator, result of second aggregator).
   */
